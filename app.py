...
# Add this at the very top after your imports
...
def create_pdf(data):
    class StyledPDF(FPDF):
        def header(self):
            ...
            self.cell(0, 10, safe("MEERUT INSTITUTE OF TECHNOLOGY"), ln=True, align="C", fill=True)
            self.set_font("Arial", 'B', 11)
            self.cell(0, 9, safe("Student Registration Form"), ln=True, align="C")
            self.ln(3)
        ...
        def footer(self):
            ...
            self.cell(0, 8, safe("Generated by MIT Online Portal | Page " + str(self.page_no())), align="C")

        def section_title(self, title):
            self.set_font("Arial", 'B', 10)
            self.set_fill_color(230, 240, 255)
            self.set_text_color(0)
            self.cell(0, 6, safe(f"  {title}"), ln=True, fill=True)
            self.ln(1)

        def field(self, label, val, w1=30, w2=60, h=5.5):
            self.set_font("Arial", 'B', 9)
            self.set_text_color(40, 40, 40)
            self.cell(w1, h, safe(label), border=0)
            self.set_font("Arial", '', 9)
            self.set_text_color(0)
            self.cell(w2, h, safe(val), border=0)
    ...
    pdf.cell(0, 8, safe(f"Unique ID: {data.get('Unique ID', '-')}"), ln=True, align="R")
    ...
    pdf.cell(35, 5.5, safe("Address:"), border=0)
    pdf.set_font("Arial", '', 9)
    pdf.multi_cell(0, 5.5, safe(data["Address"]))
    ...
    pdf.newline()
    pdf.set_font("Arial", '', 9)
    pdf.multi_cell(0, 5, safe("10th Subjects: MATH | SCIENCE | ENGLISH | HINDI | SOCIAL SCIENCE | COMPUTER"))
    pdf.multi_cell(0, 5, safe("12th Subjects: MATHS/BIO | PHYSICS | CHEMISTRY | COMPUTER | ENGLISH | PCM/PCB %"))
    ...
    pdf.set_font("Arial", '', 9)
    pdf.cell(0, 6, safe("Date: ____ / ____ / ______"), ln=True)
    pdf.cell(0, 6, safe("Candidate Signature: ___________________________"), ln=True)
    pdf.ln(4)

    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, safe("Admission Source:"), ln=True)
    pdf.set_font("Arial", size=12)
    direct = data.get("Admission Source") == "Direct"
    ccb = data.get("Admission Source") == "CCB"
    pdf.cell(0, 8, safe(f"[{'*' if direct else ' '}] Direct    [{'*' if ccb else ' '}] CCB"), ln=True)
    pdf.ln(5)

    pdf.set_font("Arial", 'B', 10)
    pdf.cell(0, 6, safe("FOR OFFICE USE"), ln=True)
    pdf.set_font("Arial", '', 9)
    pdf.multi_cell(0, 8, safe("COMMENTS/REMARKS:\n\n"))
    pdf.cell(120, 8, safe("Chairman"), ln=0)
    pdf.cell(0, 8, safe("Admission Counselor"), ln=True)

    # Output PDF and return BytesIO object
